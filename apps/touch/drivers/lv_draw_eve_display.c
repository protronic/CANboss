 ...
 *
 * Lokale Kopie von LVGLs drivers/draw/eve/lv_draw_eve_display.c mit zwei
 * Fixes gegen den Upstream-Stand (LVGL 9.5 / west-Modul):
 *
 * 1. flush_cb liess nach DISPLAY/SWAP den CMD-Burst offen
 *    (EVE_start_cmd_burst ohne Partner). Touch-Reads und Diagnose-
 *    memReads zwischen den Frames landeten mitten in der CMDB-SPI-
 *    Transaktion, korrumpierten die Displayliste und konnten den
 *    Koprozessor faulten.
 *
 * 2. EVE_execute_cmd() wartet endlos auf E_OK. Liest SPI nach einer
 *    Korruption dauerhaft Muell (z.B. 0xffff), liefert EVE_busy() immer
 *    EVE_FAULT_RECOVERED (nie E_OK) - UI und Shell hängen. Deshalb hier
 *    ein Timeout um den Swap-Wait.
 *
 * Bei LVGL-Updates diese Datei gegen Upstream diffen.
 */
...
/* Pfade relativ zu modules/lib/gui/lvgl/src (Zephyr-Include). */
#include "drivers/draw/eve/lv_draw_eve_display.h"
#if LV_USE_DRAW_EVE
...
static void flush_cb(lv_display_t * disp, const lv_area_t * area, uint8_t * px_map)
{
    if(lv_display_flush_is_last(disp)) {
        ...
        /* Nicht EVE_execute_cmd(): das wartet ohne Timeout und bleibt bei
         * dauerhaftem Fault-Read in der Recovery-Schleife haengen. Auch
         * keinen neuen Burst oeffnen - Upstream tat das und liess CS bis
