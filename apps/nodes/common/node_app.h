 ...
 * sim wird alle CB_NODE_SIM_INTERVAL_MS unter CO_LOCK_OD mit der
 * Gesamtlaufzeit in ms aufgerufen und schreibt direkt in die
 * OD-RAM-Strukturen (TPDOs senden per Event-Timer aus der EDS). */
void node_app_run(const char* name, OD_t* od, const CO_config_t* config, uint8_t node_id,
                  void (*sim)(uint32_t uptime_ms));
