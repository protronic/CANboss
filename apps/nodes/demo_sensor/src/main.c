...
sim(uint32_t t_ms) {
    ...
    demo_sensor_RAM.x2300_klima.temperatur = 21.0f + 3.0f * sinf(t * 0.05f) + 0.1f * sinf(t * 0.9f);
    ...
    uint8_t alarm = 0;
    if (demo_sensor_RAM.x2300_klima.temperatur > demo_sensor_PERSIST_COMM.x2304_grenzwerte.temperaturMax) {
        alarm |= 0x01u;
    }
    if (demo_sensor_RAM.x2300_klima.relativeFeuchte > demo_sensor_PERSIST_COMM.x2304_grenzwerte.relativeFeuchteMax) {
        alarm |= 0x02u;
    }
