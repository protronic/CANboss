 ...
 * CO_driver_target.h
 *
 * CANopenNode-Zielkonfiguration der CANbossTouch-Zephyr-App
 * (Pendant zu Middlewares/CANopen/CANopenNode_STM32/CO_driver_target.h
 * fuer den STM32-Build). Der Buszugriff laeuft ueber die
 * Backend-Schnittstelle src/can_if.h (Zephyr-CAN-API).
 *
 * Threads (src/osal.h):
 *   - RX-Thread:       cb_can_recv() -> CO_CANrxDispatch()
 *   - Mainline-Thread: CO_process() + SYNC/RPDO/TPDO
 *   - SDO-Worker:      canboss_sdo-Auftraege (LVGL-UI) und Berry-Skripte
 *
 * Die CO_LOCK_*-Makros sind mit echten Mutexen belegt
