 ...
 * reicht ndjson.c hierher weiter - Raw-CAN parallel zum laufenden
 ...
 * ndjson_poll() (slcan_pump), interleaved mit den
...
/* Byte-Senke fuer Antworten/Frames registrieren (ndjson_init). */
void slcan_init(void (*out)(const char* buf, size_t len));

/* Eine Kommandozeile (ohne CR/LF, NUL-terminiert) verarbeiten;
 * nur aus dem poll-Thread rufen. */
void slcan_line(const char* line, size_t len);

/* Empfangenen Frame einreihen (Raw-RX-Hook, beliebiger Kontext);
 * ohne offenen Kanal ein No-op. */
void slcan_rx(uint16_t ident, const uint8_t* data, uint8_t dlc, bool rtr);

/* Gepufferte RX-Frames ausgeben; zyklisch aus ndjson_poll(). */
void slcan_pump(void);
